...
export default function CourseDetailClient({ course, pageData }: CourseDetailClientProps) {
    ...
    const shareText = t("share.text", { title: course.title });
    const handleCopyLink = async () => {
        try {
            ...
            error(t("toast.linkCopyError"));
        }
    };
    const shareOptions = [
        {
            name: t("share.options.twitter"),
    ...
    const handleApplyCoupon = (e: React.FormEvent) => {
        ...
        setTimeout(() => {
            ...
            if (couponCode.toUpperCase() === 'WELCOME50') {
                success(t("toast.couponSuccess"));
            } else {
                error(t("toast.couponError"));
            }
        }, 1500);
    };
    ...
    const handleLeadSubmit = async (e: React.FormEvent) => {
        ...
        if (res.success) {
            ...
            error(res.error || t("toast.messageError"));
        }
    };
    const handleReviewSubmit = async (e: React.FormEvent) => {
        ...
        setSubmitting(false);
        if (res.success) {
            success(t("toast.reviewSuccess"));
            setShowReviewModal(false);
            router.refresh();
        } else {
            error(res.error || t("toast.reviewError"));
        }
    };
