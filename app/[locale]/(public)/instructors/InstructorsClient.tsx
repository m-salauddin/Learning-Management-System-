    ...
    const filteredInstructors =
        selectedCategory === categories[0]
            ...
            : instructors.filter((instructor) => {
                const normalizedCategory = instructor.category.toLowerCase();
                const normalizedSelected = selectedCategory.toLowerCase();
                if (normalizedSelected === t("categories.fullstack").toLowerCase()) return normalizedCategory.includes('full stack');
                if (normalizedSelected === t("categories.frontend").toLowerCase()) return normalizedCategory.includes('frontend');
                if (normalizedSelected === t("categories.backend").toLowerCase()) return normalizedCategory.includes('backend');
                if (normalizedSelected === t("categories.devops").toLowerCase()) return normalizedCategory.includes('devops');
                if (normalizedSelected === t("categories.mobile").toLowerCase()) return normalizedCategory.includes('mobile');
                if (normalizedSelected === t("categories.aiml").toLowerCase()) return normalizedCategory.includes('ai/ml');
                return normalizedCategory.includes(normalizedSelected);
            });
    return (
        <div className="min-h-screen bg-background relative overflow-hidden">
            ...
            <div className="container mx-auto px-4 pt-40 pb-24 relative z-10">
                ...
                <div className="text-center max-w-4xl mx-auto mb-20 space-y-8">
                    ...
                    >
                        {t("title1")} <span className="bg-clip-text text-transparent bg-linear-to-r from-primary via-purple-500 to-blue-500 drop-shadow-sm">{t("title2")}</span> <br />
                        {t("title3")}
                    </motion.h1>
                    <motion.p
                        initial={{ opacity: 0, y: 20 }}
                ...
                >
                    {categories.map((category) => (
                        <button
                            ...
                            className={cn(
                                "relative px-6 py-2.5 rounded-full text-sm font-bold transition-all duration-300 border overflow-hidden group cursor-pointer",
                                        ...
                                        <div className="mt-auto pt-4 border-t border-border flex items-center justify-between">
                                            <div className="flex items-center gap-6">
                                                <div className="flex flex-col">
                                                    <span className="text-[10px] text-muted-foreground uppercase tracking-wider mb-0.5">{t("card.courses")}</span>
                                                    <span className="text-base font-bold text-foreground">
                                                        {instructor.stats.courses}
                                                    </span>
                                                </div>
                                                <div className="flex flex-col">
                                                    <span className="text-[10px] text-muted-foreground uppercase tracking-wider mb-0.5">{t("card.students")}</span>
                                                    <span className="text-base font-bold text-foreground">
                                                        {instructor.stats.students.toLocaleString()}
                                                    </span>
                                                </div>
                                            </div>
                                            <Button size="sm" className="rounded-xl px-5 group/btn bg-primary text-primary-foreground hover:bg-primary/90 cursor-pointer">
                                                {t("card.profile")}
