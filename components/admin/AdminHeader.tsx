...
export function AdminHeader({ onMobileMenuOpen }: { onMobileMenuOpen: () => void }) {
    const pathname = usePathname();
    const { user } = useAppSelector((state) => state.auth);

    const pathSegments = pathname?.split("/").filter(Boolean) || [];
    const lastSegment = pathSegments[pathSegments.length - 1];
    const formattedTitle = lastSegment === "admin" ? "Admin Overview" : lastSegment.charAt(0).toUpperCase() + lastSegment.slice(1).replace(/-/g, " ");
    ...
    return (
        <header className="h-20 px-6 flex items-center justify-between border-b border-border/50 bg-background/50 backdrop-blur-xl sticky top-0 z-30">
            ...
            <div className="flex items-center gap-2 sm:gap-4">
                <div className="hidden md:flex items-center px-3 py-2 rounded-xl bg-red-500/5 border border-red-500/10 focus-within:ring-2 focus-within:ring-red-500/20 transition-all w-80">
                    <Search className="w-4 h-4 text-red-500/50" />
                    <input
                        type="text"
                        placeholder="Search users, courses, invoices..."
