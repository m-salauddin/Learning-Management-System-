    ...
    let html = text.replace(/```(\w*)\n?([\s\S]*?)```/g, (_, lang, code) => {
        ...
        codeBlocks.push(
            `<div class="my-3 rounded-lg overflow-hidden border border-slate-300 dark:border-white/10">` +
                ...
                `<pre class="p-3 bg-slate-50 dark:bg-slate-900 overflow-x-auto"><code class="text-[11px] font-mono text-slate-700 dark:text-slate-300 whitespace-pre">${escaped}</code></pre>` +
            `</div>`
        );
