                ...
                <div className="flex flex-col gap-3">
                    <div className="flex gap-2">
                        ...
                        <div className="flex gap-1.5 px-1">
                            <button type="button" onClick={() => updateLesson(mIndex, lIndex)} className="p-2 rounded-xl bg-emerald-500 text-white hover:bg-emerald-600 transition-all active:scale-90"><Check className="w-3.5 h-3.5" /></button>
                            <button type="button" onClick={() => setEditingLesson(null)} className="p-2 rounded-xl bg-rose-500/10 text-rose-500 hover:bg-rose-500 hover:text-white transition-all active:scale-90"><X className="w-3.5 h-3.5" /></button>
                        </div>
                    </div>
                        ...
                        {editingModule?.index === mIndex ? (
                            <div className="flex items-center gap-2 pr-4">
                                ...
                                <div className="flex gap-1">
                                    <button type="button" onClick={(e) => { e.stopPropagation(); updateModule(mIndex); }} className="p-1.5 rounded-lg bg-emerald-500/10 text-emerald-600 hover:bg-emerald-600 hover:text-white transition-all"><Check className="w-4 h-4" /></button>
                                    <button type="button" onClick={(e) => { e.stopPropagation(); setEditingModule(null); }} className="p-1.5 rounded-lg bg-red-500/10 text-red-600 hover:bg-red-600 hover:text-white transition-all"><X className="w-4 h-4" /></button>
                                </div>
                            </div>
                        ) : (
                ...
                {isActive && (
                    ...
                    >
                        <div className="p-4 space-y-3 bg-slate-50/50 dark:bg-black/20">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-3 p-4 bg-white dark:bg-slate-950/50 rounded-xl border border-slate-100 dark:border-white/5">
                                <div className="space-y-1.5">
                                    <label className={cn(labelClasses, "mb-1.5 scale-95 origin-left")}>Lesson Title</label>
                                    <input
                                        value={lessonInput.title}
                                        onChange={(e) => setLessonInput((prev: any) => ({ ...prev, title: e.target.value }))}
                                        placeholder="What will they learn?"
                                        className={cn(inputClasses, "h-10 text-sm focus:border-violet-500/50 focus:ring-2 focus:ring-violet-500/20 bg-white dark:bg-slate-950 rounded-xl")}
                                    />
                                </div>
                                <div className="space-y-1.5">
                                    <label className={cn(labelClasses, "mb-1.5 scale-95 origin-left")}>Video URL</label>
                                    <div className="flex gap-2">
                                        <input
                                            value={lessonInput.video_url}
                                            onChange={(e) => setLessonInput((prev: any) => ({ ...prev, video_url: e.target.value }))}
                                            placeholder="Paste link here..."
                                            className={cn(inputClasses, "h-10 text-xs font-mono focus:border-violet-500/50 focus:ring-2 focus:ring-violet-500/20 bg-white dark:bg-slate-950 rounded-xl")}
                                        />
                                        <button
                                            type="button"
                                            onClick={() => addLessonToModule(mIndex)}
                                            className="px-3.5 bg-violet-600 hover:bg-violet-700 text-white rounded-xl text-[10px] font-bold transition-all shadow-sm active:scale-95 flex items-center justify-center gap-2 whitespace-nowrap"
                                        >
            ...
            <div className="space-y-4">
                <div className="space-y-3">
                    <div className="flex items-center gap-2.5 px-1.5">
                        ...
                        <div>
                            <h3 className="text-[12px] font-black uppercase tracking-[0.15em] text-violet-400">Curriculum Architecture</h3>
                            <p className="text-[9px] font-medium text-slate-500 dark:text-white/30 tracking-tight">Structure your course into logical modules and deep-dive lessons.</p>
                        </div>
                    </div>
                        ...
                        <div className="space-y-4">
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-200 dark:border-white/5 pb-4">
                                <div className="text-left">
                                    <h3 className="text-[11px] font-black uppercase tracking-[0.12em] text-slate-900 dark:text-white flex items-center gap-2">
                                        <Layers className="w-3.5 h-3.5 text-violet-400" />
                                        Course Modules
                                    </h3>
                                    <p className="text-[9px] font-medium text-slate-500 dark:text-white/30 tracking-tight">Add and organize your educational content</p>
                                </div>
                                <button
                                    type="button"
                                    onClick={() => setShowModuleForm(!showModuleForm)}
                                    className={cn(
                                        "px-3 py-1.5 rounded-xl text-[10px] font-bold transition-all flex items-center gap-2",
                                        showModuleForm
                                            ? "bg-rose-500/10 text-rose-500 hover:bg-rose-500 hover:text-white"
                                            : "bg-violet-600 text-white hover:bg-violet-700 shadow-sm active:scale-95"
                                    )}
                                >
                                    {showModuleForm ? <><X className="w-3.5 h-3.5" /> Close</> : <><Plus className="w-3.5 h-3.5" /> Add Module</>}
                                </button>
                            </div>
                            <AnimatePresence>
                                {showModuleForm && (
                                    <motion.div
                                        ...
                                        animate={{ height: "auto", opacity: 1 }}
                                        exit={{ height: 0, opacity: 0 }}
                                        className="overflow-hidden p-5 bg-white dark:bg-slate-950/50 rounded-xl border border-slate-200 dark:border-white/5 space-y-4 mb-4"
                                    >
                                        <div className="space-y-1.5">
                                            <label className={cn(labelClasses, "mb-1.5 scale-95 origin-left")}>New Module Title</label>
                                            <div className="flex gap-2">
                                                <input
                                                    value={moduleTitle}
                                                ...
                                                <button
                                                    type="button"
                                                    onClick={addModule}
                                                    className="px-4 bg-violet-600 hover:bg-violet-700 text-white rounded-xl text-[11px] font-bold transition-all shadow-sm active:scale-95 flex items-center gap-2 whitespace-nowrap"
                                                >
                                ...
                                {!formData.modules?.length ? (
                                    <div className="py-14 sm:py-20 flex flex-col items-center justify-center gap-4 border-2 border-dashed border-slate-200 dark:border-white/5 rounded-2xl bg-white/5 text-slate-400/50 transition-colors hover:border-violet-500/20 group/empty">
                                        <div className="w-16 h-16 rounded-full bg-violet-500/5 flex items-center justify-center border border-violet-500/10 group-hover/empty:scale-110 transition-transform">
                                            <Layers className="w-8 h-8 text-violet-500/40" />
                                        </div>
                                        <p className="text-[10px] font-black uppercase tracking-[0.25em] text-center px-6">No Curriculum Modules Added</p>
                                    </div>
                                ) : (
                                    <div className="space-y-3">
                ...
                <div className="space-y-3">
                    <div className="flex items-center gap-2.5 px-1.5">
                        ...
                        <div>
                            <h3 className="text-[12px] font-black uppercase tracking-[0.15em] text-violet-400">Practical Forge</h3>
                            <p className="text-[9px] font-medium text-slate-500 dark:text-white/30 tracking-tight">Add hands-on projects and laboratory exercises for student assessment.</p>
                        </div>
                    </div>
                    <div className="p-5 md:p-6 rounded-xl border border-slate-200 dark:border-white/5 bg-slate-50/50 dark:bg-slate-900/40 backdrop-blur-xl shadow-lg dark:shadow-2xl space-y-6 transition-all hover:bg-slate-100/50 dark:hover:bg-slate-900/50 hover:border-slate-300 dark:hover:border-white/10 group/section">
                        <div className="space-y-4">
                            <div className="pb-4 border-b border-slate-200 dark:border-white/5 flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h3 className="text-[11px] font-black uppercase tracking-[0.12em] text-slate-900 dark:text-white">Project Labs</h3>
                                    </div>
                                </div>
                                <button
                                    type="button"
                            ...
                            <AnimatePresence>
                                {showProjectForm && (
                                    <motion.div
                                        ...
                                        animate={{ height: "auto", opacity: 1 }}
                                        exit={{ height: 0, opacity: 0 }}
                                        className="overflow-hidden grid grid-cols-1 md:grid-cols-2 gap-x-5 gap-y-4 p-5 bg-white dark:bg-slate-950/50 rounded-xl border border-slate-200 dark:border-white/5"
                                    >
                                        <div className="space-y-1.5">
                                            <label className={cn(labelClasses, "mb-1.5 scale-95 origin-left")}>Project Title</label>
                                            <input
                                                value={projectInput.title}
                                                onChange={(e) => setProjectInput(prev => ({ ...prev, title: e.target.value }))}
                                                placeholder="What are they building?"
                                                className={cn(inputClasses, "h-10 text-sm focus:border-violet-500/50 focus:ring-2 focus:ring-violet-500/20 bg-white dark:bg-slate-950")}
                                            />
                                        </div>
                                        <div className="space-y-1.5">
                                            <label className={cn(labelClasses, "mb-1.5 scale-95 origin-left")}>Asset URL</label>
                                            <input
                                                ...
                                                className={cn(inputClasses, "h-10 text-sm font-mono focus:border-violet-500/50 focus:ring-2 focus:ring-violet-500/20 bg-white dark:bg-slate-950")}
                                            />
                                        </div>
                                        <div className="space-y-1.5 md:col-span-2">
                                            <label className={cn(labelClasses, "mb-1.5 scale-95 origin-left")}>Tech Stack <span className="text-[8px] font-normal lowercase opacity-50 ml-1">(comma separated)</span></label>
                                            <input
                                        ...
                                        <div className="md:col-span-2 space-y-1.5">
                                            <label className={cn(labelClasses, "mb-1.5 scale-95 origin-left")}>Project Description</label>
                                            <textarea
                                                value={projectInput.description}
                                                onChange={(e) => setProjectInput(prev => ({ ...prev, description: e.target.value }))}
                                                placeholder="Detail the technical hurdles and final outcome..."
                                                className={cn(inputClasses, "h-24 p-4 text-sm resize-none focus:border-violet-500/50 focus:ring-2 focus:ring-violet-500/20 bg-white dark:bg-slate-950 custom-scrollbar")}
                                            />
                                        </div>
                                        <div className="md:col-span-2 flex justify-end items-center gap-2 pt-2 bg-slate-50/50 dark:bg-white/2 rounded-b-xl -mx-5 -mb-5 px-5 h-14">
                                            {editIndex !== null && (
                                                <button
                                                    type="button"
                                                    onClick={() => {
                                                    ...
                                                    className="h-9 px-4 bg-slate-100 dark:bg-white/5 text-slate-600 dark:text-slate-400 rounded-lg text-[10px] font-bold transition-all hover:bg-slate-200 dark:hover:bg-white/10"
                                                >
                                                    Cancel
                                                </button>
                                            )}
                                            <button
                                                ...
                                                className="h-9 px-4 mb-2 bg-violet-600 hover:bg-violet-700 text-white rounded-lg text-[10px] font-bold transition-all shadow-sm active:scale-95 flex items-center justify-center gap-2"
                                            >
                                                <Plus className="w-3.5 h-3.5 text-white/80" /> {editIndex !== null ? 'Update Project' : 'Add Project'}
                                            </button>
                                        </div>
                                    </motion.div>
                                )}
                            ...
                            <div className="mt-2">
                                {!formData.projects?.length ? (
                                    <div className="py-14 sm:py-20 flex flex-col items-center justify-center gap-4 border-2 border-dashed border-slate-200 dark:border-white/5 rounded-2xl bg-white/5 text-slate-400/50 transition-colors hover:border-violet-500/20 group/empty">
                                        <div className="w-16 h-16 rounded-full bg-violet-500/5 flex items-center justify-center border border-violet-500/10 group-hover/empty:scale-110 transition-transform">
                                            <LayoutGrid className="w-8 h-8 text-violet-500/40" />
                                        </div>
                                        <p className="text-[10px] font-black uppercase tracking-[0.25em] text-center px-6">No Practical Forge Labs Discovered</p>
                                    </div>
                                ) : (
                                    <div className="space-y-3">
                                        <div className="flex items-center gap-2 mb-3">
                                            ...
                                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Managed Labs</span>
                                        </div>
